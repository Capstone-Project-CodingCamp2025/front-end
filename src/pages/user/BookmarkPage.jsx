...
const BookmarkPage = () => {
  ...
  return (
    <div className="px-4 py-8 mb-12 md:px-16 lg:px-24">
      ...
      {bookmarks.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {bookmarks.map((destination) => (
            <div
              key={destination.id}
              className="relative flex flex-col overflow-hidden transition-transform duration-300 bg-white shadow-lg rounded-2xl group"
            >
              {/* Area yang bisa diklik untuk navigasi */}
              <div
                className="w-full h-48 overflow-hidden md:h-56 cursor-pointer"
                onClick={() => navigateToDestination(destination.id)}
                role="link" 
                tabIndex={0} 
                onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') navigateToDestination(destination.id);}} // Trigger dengan Enter/Space
              >
                <img
              ...
              <div
                ...
                role="link"
                tabIndex={0}
                onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') navigateToDestination(destination.id);}}
              >
